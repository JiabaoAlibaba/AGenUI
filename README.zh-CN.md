...
- **数据绑定状态下发**：核心引擎在组件快照中新增 `dataBindingStatus` 元数据，区分无数据依赖、未就绪、部分就绪和完全就绪。
...
- (Android) 修复图片以 `fit: cover` 展示时超出自身边界、遮盖相邻组件的问题。
- (Core) 修复嵌套属性中的 `action` 被递归解析而破坏事件数据的问题，恢复富文本 Span 点击事件的正确处理。
