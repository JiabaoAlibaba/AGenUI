...
- **深度解析支持通配符**：`registerDeepParseProperty` 的组件名称支持传入 `"*"`，一次注册即可对所有组件类型的指定属性启用嵌套动态值解析。
- **数据绑定状态下发**：组件快照中新增 `dataBindingStatus` 元数据：`0`（无数据依赖）、`1`（未就绪）、`2`（部分就绪）、`3`（完全就绪）。即使属性值未变化，绑定状态变化也会触发组件更新。
- **(鸿蒙) 统一组件注册**：组件创建与内置测量注册统一由组件描述表驱动，所有 Surface 共用全局组件注册表；ArkTS 自定义视图的注册与注销同步到原生层。
- **(iOS) 属性更新回调语义调整**：`onPropertiesUpdate` 改为返回更新后的完整属性快照，与 Android / 鸿蒙的全量属性回调语义对齐。

### Bug 修复

- (Android) 修复图片以 `fit: cover` 展示时超出自身边界、遮盖相邻组件的问题，并新增多图并排场景的渲染回归用例，用于检查三端裁剪行为。
- (Core) 深度属性解析时保留嵌套 `action` 的原始数据，修复富文本 Span 点击事件处理异常的问题。
- (鸿蒙) 为布局更新及背景图清理等原生调用补充节点句柄判空，避免节点句柄为空时继续调用 ArkUI 接口。
- (鸿蒙) 修复混合视图创建失败时临时组件状态对象的内存泄漏问题。
